...
@Component({
  ...
  template: `
    <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      <div 
        *ngIf="isOpen" 
        class="mb-4 w-80 sm:w-96 h-96 bg-white/90 backdrop-blur-md rounded-2xl shadow-chat flex flex-col animate-fade-in"
        ...
        <div #messageContainer class="flex-1 p-4 overflow-y-auto">
          <div 
            *ngFor="let message of messages" 
            ...
            [ngClass]="{'ml-auto': message.sender === 'user'}">
            <div 
              class="p-3 rounded-2xl inline-block text-sm"
        ...
        <div class="p-3 border-t border-gray-100">
          <form (ngSubmit)="sendMessage()" class="flex items-center">
            <input
              ...
              class="flex-1 py-2 px-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-transparent"
              autofocus
            />
            <button 
              type="submit" 
              [disabled]="!newMessage.trim() || isSending"
              class="ml-2 bg-primary-500 text-white rounded-full p-2 disabled:opacity-50 flex items-center justify-center"
      ...
      <button 
        (click)="toggleChat()" 
        class="bg-primary-500 hover:bg-primary-600 w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-white transition-all duration-300 ease-in-out"
        [class.animate-pulse-slow]="!isOpen && hasUnreadMessages"
